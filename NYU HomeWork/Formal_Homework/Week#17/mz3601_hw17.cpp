...
public:
    RBTNode(T data)
        : data(data), color(RED), parent(nullptr), left(nullptr), right(nullptr){}
        //new insert is always red
    friend class RBT<T>;
    ...
    int height() const;//Professor did not store the height, he calculate the height. 
};
...
template <class T>
void RBTNode<T>::prettyPrint(int indent) const { //====>>>> 这个没懂: print function
    if (right != nullptr) {
...
//老师要求: 明白single rotation的写法 
//完成
template <class T>
void RBT<T>::singleCR(RBTNode<T> *&point) { //RR Rotation
    ...
    parent->height = max(parent->left->height, point->right->height) + 1;
}

template <class T>
void RBT<T>::singleCCR(RBTNode<T> *&point) { //LL rotation
    RBTNode<T> *grandparent = point;
    ...
    grandparent->height = max(point->left->height,point->right->height) + 1;
...
void RBT<T>::insert(const T &toInsert, RBTNode<T> *&point, RBTNode<T> *parent) 
{
    if (point == nullptr) {               // leaf location is found so insert node
        ...
        if(parent ==nullptr)
          return;
        while(parent && parent->color == RED){//parent == red
            RBTNode<T>* grandparent = parent-> parent;
            if(parent == grandparent ->left){
                RBTNode<T>* uncle = grandparent->right;
                if(uncle && uncle->color == RED)//uncle exists and == red
                {
                    //color change
                ...
                else //uncle does not exist + exists == black
                {
                    if(cur == parent->left){
